...
// allows you to experiment with the RTSP frame rate easily
...
// explicit power off is necessary for Timer Camera as the button does not power it off
// reset can be used but inconvenient
#ifdef CAMERA_MODEL_M5STACK_PSRAM
...
void setup()
{
    ...
    // initialise battery and turn on led to indicate power is on
  #ifdef CAMERA_MODEL_M5STACK_PSRAM   
    bat_init();
    pinMode(2, OUTPUT);
    digitalWrite(2, HIGH);
...
// reset can be used but inconvenient
#ifdef CAMERA_MODEL_M5STACK_PSRAM
    server.on("/poweroff", HTTP_GET, handle_power_off);
#endif
    server.begin();
#endif
...
void loop()
{
...
#ifdef ENABLE_RTSPSERVER
    uint32_t msecPerFrame = MSEC_PER_FRAME;
    ...
    if(session) {
        ...
        if(client) {
            ...
            streamer = new OVCamStreamer(&client, cam);             // our streamer for UDP/TCP based RTP transport
