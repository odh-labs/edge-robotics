...
#+begin_src bash :results outputa
cp secret.yaml quay.yaml
sed -i "s/aGVsbG93b3JsZAo=/$(echo ${QUAY_USER} | base64)/g" gitops-pipeline/quay.yaml
sed -i "s/cGxhY2Vob2xkZXIyCg==/$(echo ${QUAY_PASS} | base64)/g" gitops-pipeline/quay.yaml
oc apply --namespace edge-robotics --filename gitops-pipeline/quay.yaml
#+end_src
